...
* Реверс-прокси  **Traefik** собирает информацию о запущенных динамических DNS-именах для **.dev.company.ru*,
подключившись к докеру [**Staging**] по TCP. Также автоматически 
...
* **GitLab** также работает как Docker Registry (registry.company.ru), где хранятся образы приложений.
